...
export default function AdminCmsPage() {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated || !token) {
      router.replace(`/${locale}/login?redirectTo=/${locale}/admin/cms`);
      return;
    }
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 px-6 text-center">
        ...
        <p className="max-w-md text-sm text-[hsl(var(--muted-foreground))]">
          برای مدیریت محتوای سایت باید با حساب ادمین وارد شوید.
        </p>
        <Link
          href={`/${locale}/login`}
  ...
  return (
    <div className="min-h-screen bg-[hsl(var(--secondary))/30]">
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-8">
        <div className="mb-6">
          <h1 className="text-2xl font-bold">مدیریت محتوای سایت (CMS)</h1>
          <p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">
            ویرایش هدر، فوتر و بخش‌های صفحه فرود — تغییرات پس از انتشار عمومی می‌شود.
          </p>
        </div>
        <CmsEditor />
      </div>
