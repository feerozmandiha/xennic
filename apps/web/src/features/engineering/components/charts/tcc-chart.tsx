...
export function TCCChart({
  ...
  const { data, xDomain, yDomain, xTicks, yTicks } = useMemo(() => {
    ...
    if (!allI.length || !allT.length) {
      return {
        data: [],
        xDomain: [1, 1e5] as [number, number],
        yDomain: [0.001, 1e4] as [number, number],
        xTicks: [] as number[],
        yTicks: [] as number[],
      };
    }
        ...
        <LineChart data={data} margin={{ top: 8, right: 12, bottom: 8, left: 12 }}>
          ...
          <Tooltip
            ...
            labelFormatter={(label: any) =>
              `I = ${Number(label) >= 1000 ? `${(Number(label) / 1000).toFixed(2)} kA` : `${Number(label).toFixed(1)} A`}`
            }
          />
