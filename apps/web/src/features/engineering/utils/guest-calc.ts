...
export function calcLocal(code: string, inputs: CalcInputs) {
  ...
  switch (code) {
    ...
    case 'BASIC-002': {
      ...
      const p =
        phaseType === 'three'
          ? Math.sqrt(3) * v('voltage_v') * v('current_a') * pf
          : v('voltage_v') * v('current_a') * pf;
      return {
        status: 'success',
        result: { active_power_w: p, power_factor: pf, formula: 'P = V × I × PF', code },
      };
    }
    case 'BASIC-003': {
      const phaseType = inputs['phase_type'] ?? 'single';
      const s =
        phaseType === 'three'
          ? Math.sqrt(3) * v('voltage_v') * v('current_a')
          : v('voltage_v') * v('current_a');
    ...
    case 'BASIC-004': {
      ...
      return {
        status: 'success',
        result: { reactive_power_var: q, formula: 'Q = √(S² - P²)', code },
      };
    }
    case 'BASIC-005': {
      const p = v('active_power_w');
      ...
      return {
        ...
        result: {
          power_factor: pf,
          theta_deg: (Math.acos(Math.max(-1, Math.min(1, pf))) * 180) / Math.PI,
          formula: 'PF = P / S',
          code,
        },
      };
