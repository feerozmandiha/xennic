...
export function BillingClient() {
  ...
  const { data: dashData, isLoading: dashLoading } = useQuery({
    ...
    queryFn: () =>
      apiClient.get<{ success: boolean; data: BillingDashboard }>('/billing/dashboard'),
    enabled: !!wsId,
  });
  ...
  const { data: invData, isLoading: invLoading } = useQuery({
    ...
    queryFn: () =>
      apiClient.get<{ success: boolean; data: Invoice[]; meta: any }>('/billing/invoices?limit=10'),
    enabled: !!wsId,
  });
  ...
  const { data: payData, isLoading: payLoading } = useQuery({
    ...
    queryFn: () =>
      apiClient.get<{ success: boolean; data: Payment[]; meta: any }>('/billing/payments?limit=10'),
    enabled: !!wsId,
  });
    ...
    <div className="space-y-6 max-w-6xl">
      ...
      {isAdmin && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-[hsl(var(--primary)/0.3)] bg-[hsl(var(--primary)/0.05)] text-sm">
          ...
          <span>
            به عنوان <span className="font-semibold">ادمین سیستم</span>، دسترسی کامل به تمام محاسبات
            و ویژگی‌ها برقرار است.
          </span>
        </div>
      )}
      ...
      {!dashLoading && dash && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <StatCard
            ...
            value={`${formatPrice(dash.totalInvoiced)} تومان`}
            icon={TrendingUp}
            color="bg-blue-500"
          />
          <StatCard
            title="پرداخت شده"
            value={`${formatPrice(dash.totalPaid)} تومان`}
            icon={Wallet}
            color="bg-green-500"
            sub={`${dash.recentPayments?.length ?? 0} تراکنش`}
          />
          <StatCard
            title="در انتظار پرداخت"
            value={`${formatPrice(dash.totalPending)} تومان`}
            icon={AlertTriangle}
            color="bg-yellow-500"
          />
          {dash.totalOverdue > 0 && (
            <StatCard
              title="سررسید شده"
              value={`${formatPrice(dash.totalOverdue)} تومان`}
              icon={Ban}
              color="bg-red-500"
            />
          )}
      ...
      {section === 'plans' && (
        <div className="space-y-6">
          ...
          {plansLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {Array.from({ length: 3 }).map((_, i) => (
                <div
                  key={i}
                  className="h-96 rounded-xl bg-[hsl(var(--secondary)/0.5)] animate-pulse"
                />
              ))}
            </div>
          ) : plans.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {plans.map((plan) => {
                ...
                const PlanIcon =
                  plan.slug === 'free' ? Zap : plan.slug === 'pro' ? Crown : Building2;
                const planFeatures = (plan.features as Record<string, any>) ?? {};

                const FEATURE_LIST: { label: string; ok: boolean }[] = [];
                if (planFeatures.projects !== undefined)
                  FEATURE_LIST.push({ label: `${planFeatures.projects} پروژه`, ok: true });
                if (planFeatures.calculations_month !== undefined)
                  FEATURE_LIST.push({
                    label:
                      planFeatures.calculations_month === -1
                        ? 'محاسبات نامحدود'
                        : `${planFeatures.calculations_month} محاسبه/ماه`,
                    ok: true,
                  });
                if (planFeatures.ai_requests_month !== undefined)
                  FEATURE_LIST.push({
                    label:
                      planFeatures.ai_requests_month === -1
                        ? 'AI نامحدود'
                        : `${planFeatures.ai_requests_month} AI/ماه`,
                    ok: true,
                  });
                if (planFeatures.storage_gb !== undefined)
                  FEATURE_LIST.push({
                    label:
                      planFeatures.storage_gb === -1
                        ? 'فضای نامحدود'
                        : `${planFeatures.storage_gb} گیگابایت`,
                    ok: true,
                  });
                if (planFeatures.api_access)
                  FEATURE_LIST.push({
                    label: `دسترسی API سطح ${planFeatures.api_access}`,
                    ok: true,
                  });
                if (planFeatures.report_formats)
                  FEATURE_LIST.push({
                    label: `گزارش‌های ${planFeatures.report_formats}`,
                    ok: true,
                  });
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isHighlight
                        ? 'border-[hsl(var(--primary)/0.5)] shadow-[0_0_0_1px_hsl(var(--primary)/0.15),0_4px_24px_hsl(var(--primary)/0.08)]'
                  ...
                  >
                    ...
                    <div
                      className={cn(
                        'p-6 rounded-t-xl',
                        isHighlight ? 'bg-[hsl(var(--primary)/0.04)]' : '',
                      )}
                    >
                      ...
                      <div className="mb-5">
                        {plan.slug === 'enterprise' ? (
                          ...
                          <div className="flex items-end gap-1.5">
                            ...
                            <span className="text-xs text-[hsl(var(--muted-foreground))] pb-1">
                              تومان / ماه
                            </span>
                          </div>
                        )}
                      ...
                      {plan.slug === 'enterprise' ? (
                        <button
                          onClick={() => window.open('mailto:sales@xennic.ir', '_blank')}
                        ...
                        <button
                          onClick={() =>
                            router.push(`/${locale}/billing/checkout?plan=${plan.slug}`)
                          }
                          className={cn(
                            'w-full h-10 rounded-lg text-sm font-semibold transition-opacity flex items-center justify-center gap-1.5',
                            isHighlight
                        ...
                        >
                          {plan.slug === 'pro' ? (
                            <>
                              <Crown className="h-3.5 w-3.5" /> ارتقا به Pro
                    ...
                    <div className="p-6 pt-5 flex-1 border-t border-[hsl(var(--border))] rounded-b-xl">
                      <p className="text-[10px] font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wider mb-3">
                        امکانات
                      </p>
                          ...
                          <li key={f.label} className="flex items-start gap-2 text-xs">
                            <CheckCircle2
                              className={cn(
                                ...
                                f.ok ? 'text-green-600' : 'text-[hsl(var(--muted-foreground)/0.3)]',
                              )}
                            />
                            <span
                              className={cn(
                                !f.ok && 'text-[hsl(var(--muted-foreground)/0.5)] line-through',
                              )}
                            >
                              {f.label}
                            </span>
          ...
          <p className="text-center text-xs text-[hsl(var(--muted-foreground))]">
            تمام پلن‌ها شامل پشتیبانی اولیه هستند · برای سوال با{' '}
            <a href="mailto:support@xennic.ir" className="underline">
              support@xennic.ir
            </a>{' '}
            تماس بگیرید
          </p>
                ...
                invoices.map((inv) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-medium tabular-nums">
                      {formatPrice(inv.totalAmount)} تومان
                    </td>
                    <td className="px-4 py-3 text-xs text-[hsl(var(--muted-foreground))]">
                      {new Date(inv.issuedAt).toLocaleDateString('fa-IR')}
                    </td>
                    <td className="px-4 py-3 text-xs text-[hsl(var(--muted-foreground))]">
                      {inv.dueAt ? new Date(inv.dueAt).toLocaleDateString('fa-IR') : '—'}
                    </td>
                  </tr>
                ))
              )}
