...
function NewConsultationForm({
  ...
  const mutation = useMutation({
    mutationFn: () =>
      apiClient.post<ConsultationResponse>('/consultations', {
        ...form,
        tags: form.tags
          .split(',')
          .map((t) => t.trim())
  ...
  const inputCls =
    'w-full px-3 py-2 rounded-[var(--radius-lg)] border border-[hsl(var(--border))] bg-[hsl(var(--background))] text-sm outline-none focus:border-[hsl(var(--primary))] focus:ring-1 focus:ring-[hsl(var(--primary)/0.3)] transition-all';
          ...
          {c.tags.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {c.tags.map((tag) => (
                <span
                  key={tag}
                  className="text-[10px] px-1.5 py-0.5 rounded bg-[hsl(var(--secondary))]"
                >
                  #{tag}
                </span>
              ))}
                ...
                <div className="flex items-center gap-2 mb-1.5">
                  <span
                    className={cn(
                      ...
                      r.isExpert && 'text-[hsl(var(--primary))]',
                    )}
                  >
                    {r.authorName}
                  </span>
  ...
  return (
    ...
    >
      <CardContent className="p-4 space-y-2">
        <div className="flex items-start justify-between gap-3">
          <h3 className="text-sm font-semibold leading-relaxed flex-1 line-clamp-2">
            {item.title}
          </h3>
          <span
            className={cn(
...
export function ConsultationsClient() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      apiClient.get<ConsultationsResponse>(
      ...
      {isLoading ? (
        <div className="space-y-3">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="h-24 rounded-[var(--radius-xl)] bg-[hsl(var(--secondary)/0.5)] animate-pulse"
            />
          ))}
        </div>
            ...
            {[
              {
                icon: Bot,
                title: 'پاسخ AI فوری',
                desc: 'Xennic AI با Groq llama-3.3 پاسخ تخصصی مهندسی برق ارائه می‌دهد',
              },
              {
                icon: Zap,
                title: 'استانداردها',
                desc: 'پاسخ‌ها با ذکر IEC، IEEE و NEMA ارائه می‌شوند',
              },
              {
                icon: MessageSquare,
                title: 'مشاوره تخصصی',
                desc: 'کابل، ترانس، حفاظت، کیفیت توان، زمین، موتور و تعرفه',
              },
