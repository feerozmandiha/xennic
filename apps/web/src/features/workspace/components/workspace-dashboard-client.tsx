...
export function WorkspaceDashboardClient() {
  ...
  const storageUsed = storageBytes > 0 ? (storageBytes / (1024 * 1024 * 1024)).toFixed(1) : '0';
            ...
            {loadAll ? (
              ...
              <ul className="divide-y divide-[hsl(var(--border))]">
                {projects.slice(0, 5).map((p: any) => (
                  <li
                    key={p.id}
                    className="flex items-center gap-3 px-5 py-3 group hover:bg-[hsl(var(--secondary)/0.3)] transition-colors"
                  >
