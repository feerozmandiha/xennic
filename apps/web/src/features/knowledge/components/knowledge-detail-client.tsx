...
import { useKnowledgeTier } from '../hooks/use-knowledge-tier';
import { TierPaywall } from './tier-paywall';
...
export function KnowledgeDetailClient({ articleId }: Props) {
  ...
  const requiredTier = (article?.accessTier as AccessTier) ?? 'free';
  const { canAccess, tier: currentTier } = useKnowledgeTier();
  const hasAccess = article ? canAccess(requiredTier) : true;
  ...
  return (
    <div>
      <PageHeader
        ...
        description={
          <span className="flex items-center gap-2">
            ...
            {requiredTier !== 'free' && (
              <span
                className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${TIER_BADGE_COLORS[requiredTier]}`}
              >
                {TIER_LABEL_FA[requiredTier]}
              </span>
            )}
      ...
      {!hasAccess ? (
        <TierPaywall required={requiredTier} current={currentTier} />
      ) : (
