...
import { LandingCmsEditor } from './landing-cms-editor';
...
const SECTIONS = [
  ...
  { key: 'landing-cms', label: 'تنظیمات لندینگ', icon: FileEdit },
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard
          ...
          sub={`${s?.users?.active ?? 0} فعال`}
          icon={Users}
          color="bg-blue-500"
          trend={s?.users?.new_30d}
        />
        <StatCard
          title="Workspace ها"
          value={s?.workspaces?.total ?? 0}
          sub={`${s?.workspaces?.new_30d ?? 0} ماه جاری`}
          icon={Building2}
          color="bg-purple-500"
        />
        <StatCard
          title="محاسبات"
          value={s?.calculations?.total ?? 0}
          sub={`${s?.calculations?.new_30d ?? 0} ماه جاری`}
          icon={Zap}
          color="bg-orange-500"
        />
        <StatCard
          title="مشاوره‌ها"
          value={s?.consultations?.total ?? 0}
          sub={`${s?.consultations?.pending ?? 0} در انتظار`}
          icon={MessageSquare}
          color="bg-yellow-500"
        />
        <StatCard
          title="درآمد ماهانه"
          value={`${((s?.revenue?.monthly ?? 0) / 1_000_000).toFixed(1)}M ریال`}
          sub="ماه جاری"
          icon={CreditCard}
          color="bg-green-500"
        />
          ...
          <thead className="bg-[hsl(var(--secondary)/0.5)] sticky top-0">
            <tr>
              {['ایمیل', 'نام', 'Workspace', 'وضعیت', 'ادمین', 'عملیات'].map((h) => (
                <th
                  key={h}
                  className="text-right text-xs font-semibold text-[hsl(var(--muted-foreground))] px-4 py-3 whitespace-nowrap"
                >
                  {h}
                </th>
              ))}
              ...
              users.map((u: any) => (
                <tr key={u.id} className="hover:bg-[hsl(var(--secondary)/0.3)] transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <button
                      onClick={() =>
                        updateUser.mutate({ id: u.id, body: { isAdmin: !u.is_admin } })
                      }
                      disabled={updateUser.isPending}
                      title={
                        u.is_admin ? 'کلیک کنید تا دسترسی ادمین حذف شود' : 'کلیک کنید تا ادمین شود'
                      }
...
function WorkspacesSection() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      apiClient.get<any>(`/admin/workspaces?limit=50&search=${encodeURIComponent(search)}`),
  });
                  ...
                  <tr key={ws.id} className="hover:bg-[hsl(var(--secondary)/0.3)]">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={cn(
                          'text-[10px] font-semibold px-2 py-0.5 rounded-full',
                          ws.plan_slug === 'enterprise'
                            ? 'bg-purple-100 text-purple-700'
                            : ws.plan_slug === 'pro'
                              ? 'bg-blue-100 text-blue-700'
                              : ws.plan_slug === 'starter'
                                ? 'bg-green-100 text-green-700'
                    ...
                    <td className="px-4 py-3 text-[hsl(var(--muted-foreground))]">
                      {Number(ws.calc_count ?? 0).toLocaleString('fa-IR')}
                    </td>
                    <td className="px-4 py-3">
                      <select
...
function PlansSection() {
  ...
  const inputCls =
    'w-full px-2 py-1 text-sm rounded border border-[hsl(var(--border))] bg-[hsl(var(--background))] outline-none focus:border-[hsl(var(--primary))]';
    ...
    <div className="space-y-4">
      {isLoading
        ...
        : plans.map((plan: any) => (
            <Card
              key={plan.id}
              className={cn(editing === plan.id && 'border-[hsl(var(--primary)/0.4)]')}
            >
              <CardContent className="p-5">
                  ...
                  <div className="flex-1 space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      {plan.subscriber_count > 0 && (
                        <span className="text-xs text-[hsl(var(--muted-foreground))]">
                          {Number(plan.subscriber_count).toLocaleString('fa-IR')} مشترک
                        </span>
                      )}
                    </div>
                    ...
                    {editing === plan.id ? (
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        <div>
                          <label className="text-[10px] text-[hsl(var(--muted-foreground))] block mb-0.5">
                            قیمت ماهانه (ریال)
                          </label>
                          <input
                            type="number"
                        ...
                        <div>
                          <label className="text-[10px] text-[hsl(var(--muted-foreground))] block mb-0.5">
                            محاسبات/ماه (-1=نامحدود)
                          </label>
                          <input
                            type="number"
                            className={inputCls}
                            value={
                              editData.features?.calculations_month ??
                              plan.features?.calculations_month ??
                              10
                            }
                            onChange={(e) =>
                              setEditData((d: any) => ({
                                ...
                                features: {
                                  ...
                                  calculations_month: Number(e.target.value),
                                },
                              }))
                            }
                          />
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className={inputCls}
                            value={
                              editData.features?.ai_requests_month ??
                              plan.features?.ai_requests_month ??
                              5
                            }
                            onChange={(e) =>
                              setEditData((d: any) => ({
                                ...
                                features: {
                                  ...(d.features ?? plan.features),
                                  ai_requests_month: Number(e.target.value),
                                },
                              }))
                            }
                          />
                      ...
                      <div className="flex flex-wrap gap-4 text-xs text-[hsl(var(--muted-foreground))]">
                        <span>
                          💰 {Number(plan.monthly_price).toLocaleString('fa-IR')} ریال/ماه
                        </span>
                        <span>
                          📊{' '}
                          {plan.features?.calculations_month === -1
                            ? 'نامحدود'
                            : plan.features?.calculations_month}{' '}
                          محاسبه
                        </span>
                        <span>
                          🤖{' '}
                          {plan.features?.ai_requests_month === -1
                            ? 'نامحدود'
                            : plan.features?.ai_requests_month}{' '}
                          AI
                        </span>
                        <span>
                          📁 {plan.features?.projects === -1 ? 'نامحدود' : plan.features?.projects}{' '}
                          پروژه
                        </span>
...
function ConsultationsSection() {
  ...
  const reply = useMutation({
    mutationFn: () =>
      apiClient.post(`/admin/consultations/${selected.id}/reply`, { content: replyText }),
    onSuccess: () => {
  ...
  const close = useMutation({
    mutationFn: (id: string) =>
      apiClient.patch(`/admin/consultations/${id}/status`, { status: 'closed' }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'consultations'] });
    ...
    <div className="space-y-4">
      <div className="flex gap-2 flex-wrap">
        ...
        <Badge variant="secondary" className="mr-auto">
          {data?.meta?.total ?? tickets.length} تیکت
        </Badge>
      </div>
      ...
      <div className="space-y-2">
        {isLoading
          ? Array.from({ length: 5 }).map((_, i) => (
              <div
                key={i}
                className="h-20 rounded-xl bg-[hsl(var(--secondary)/0.5)] animate-pulse"
              />
            ))
          : tickets.map((t: any) => (
              <Card
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={cn('text-[10px] font-bold', PRIORITY_COLOR[t.priority])}>
...
function NotificationsSection() {
  ...
  const send = useMutation({
    mutationFn: () =>
      apiClient.post<{ success: boolean; sent: number }>('/admin/notifications/broadcast', form),
    onSuccess: (res) => {
        ...
        <CardContent className="space-y-3">
          <div>
            ...
            <input
              ...
              placeholder="مثال: بروزرسانی سیستم در ساعت ۲۴"
              className={inputCls}
            />
          </div>
          <div>
            ...
            <textarea
              ...
              rows={4}
              placeholder="جزئیات اعلان را بنویسید..."
...
function SettingsSection() {
  ...
  const { data } = useQuery({
    ...
    queryFn: () =>
      apiClient.get<{ success: boolean; data: Record<string, string> }>('/admin/settings'),
  });
          ...
          <div className="pt-2 border-t border-[hsl(var(--border)/0.5)] space-y-3">
            {TOGGLES.map((t) => (
              <div key={t.key} className="flex items-center justify-between">
                ...
                <button
                  onClick={() =>
                    setForm((p) => ({ ...p, [t.key]: p[t.key] === 'true' ? 'false' : 'true' }))
                  }
    ...
    <div className="space-y-4">
        ...
        <div className="rounded-xl border border-[hsl(var(--primary)/0.3)] bg-[hsl(var(--primary)/0.03)] p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold flex items-center gap-2">
              ...
              {editing === '__new__'
                ? `ایجاد ${currentType.label} جدید`
                : `ویرایش ${currentType.label}`}
            </h3>
            <button
              onClick={() => {
                setEditing(null);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            <div>
              <label className="text-[10px] font-medium text-[hsl(var(--muted-foreground))] block mb-0.5">
                Slug *
              </label>
              <input
                value={form.slug ?? ''}
      ...
      <div className="rounded-xl border border-[hsl(var(--border))] overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-[hsl(var(--secondary)/0.5)]">
            <tr>
              <th className="text-right text-xs font-semibold text-[hsl(var(--muted-foreground))] px-4 py-3 w-12">
                #
              </th>
              {type === 'category' && (
...
function ApiKeysSection() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-xs text-[hsl(var(--muted-foreground))]">
          مدیریت کلیدهای API دسترسی به سرویس‌های Xennic
        </p>
        <button
      ...
      {newKeyValue && (
        <div className="rounded-xl border border-green-300 bg-green-50 p-4 space-y-2">
          ...
          <p className="text-xs text-green-600">
            این کلید فقط یک بار نمایش داده می‌شود. آن را در جای امنی ذخیره کنید.
          </p>
          <div className="flex items-center gap-2">
            <code className="flex-1 px-3 py-2 bg-white rounded-lg text-sm font-mono border border-green-200 ltr text-left break-all">
              {newKeyValue}
            </code>
            <button
              onClick={() => {
                ...
                <tr key={k.id} className="hover:bg-[hsl(var(--secondary)/0.3)] transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => {
                          if (confirm('آیا از حذف این کلید مطمئن هستید؟')) deleteKey.mutate(k.id);
                        }}
...
function WebhooksSection() {
  ...
  const updateWebhook = useMutation({
    mutationFn: ({ id, body }: { id: string; body: any }) =>
      apiClient.patch(`/webhooks/${id}`, body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'webhooks'] });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-xs text-[hsl(var(--muted-foreground))]">
          مدیریت Webhook‌های ارسال رویداد به سرویس‌های خارجی
        </p>
        <button
          onClick={startCreate}
          className="flex items-center gap-1.5 h-9 px-4 rounded-lg bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] text-sm hover:opacity-90"
      ...
      {showCreate && (
        <div className="rounded-xl border border-[hsl(var(--primary)/0.3)] bg-[hsl(var(--primary)/0.03)] p-4 space-y-3">
          ...
          <div>
            <label className="text-[10px] font-medium text-[hsl(var(--muted-foreground))] block mb-1.5">
              رویدادها *
            </label>
            ...
            {isLoading ? (
              ...
              webhooks.map((w: any) => (
                <tr key={w.id} className="hover:bg-[hsl(var(--secondary)/0.3)] transition-colors">
                  <td
                    className="px-4 py-3 font-mono text-xs max-w-[200px] truncate ltr text-left"
                    title={w.url}
                  >
                    {w.url}
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1">
                      {(w.events ?? []).slice(0, 3).map((ev: string) => (
                        <span
                          key={ev}
                          className="text-[10px] px-1.5 py-0.5 rounded bg-[hsl(var(--secondary)/0.5)] text-[hsl(var(--muted-foreground))]"
                        >
                          {ev}
                        </span>
                      ))}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => {
                          if (confirm('آیا از حذف این Webhook مطمئن هستید؟'))
...
function FeatureFlagsSection() {
  ...
  const toggleFlag = useMutation({
    mutationFn: ({ id, enabled }: { id: string; enabled: boolean }) =>
      apiClient.patch(`/admin/feature-flags/${id}/toggle`, { enabled }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'feature-flags'] });
    },
  ...
  const updateFlag = useMutation({
    mutationFn: ({ id, body }: { id: string; body: any }) =>
      apiClient.patch(`/admin/feature-flags/${id}`, body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'feature-flags'] });
      toast.success('Feature flag بروزرسانی شد');
      closeForm();
    },
  ...
  const deleteFlag = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'feature-flags'] });
      toast.success('Feature flag حذف شد');
    },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-xs text-[hsl(var(--muted-foreground))]">
          مدیریت Feature Flagهای سراسری پلتفرم
        </p>
        <button
          onClick={startCreate}
          className="flex items-center gap-1.5 h-9 px-4 rounded-lg bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] text-sm hover:opacity-90"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                className={inputCls}
                placeholder="فعال‌سازی ماشین حساب هوش مصنوعی"
              />
            </div>
            <div>
              <label className="text-[10px] font-medium text-[hsl(var(--muted-foreground))] block mb-0.5">
              ...
              <div className="flex gap-1.5 mt-1">
                {[
                  ...
                  <button
                    key={s.key}
                    onClick={() =>
                      setForm((f: any) => ({ ...f, scope: s.key, planId: '', workspaceId: '' }))
                    }
                    className={cn(
                      'h-7 px-3 rounded-lg text-[10px] font-medium border transition-all',
                      form.scope === s.key
                        ? 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] border-[hsl(var(--primary))]'
            ...
            {form.scope === 'plan' && (
              <div>
                ...
                <input
                  value={form.planId ?? ''}
                  onChange={(e) => setForm((f: any) => ({ ...f, planId: e.target.value }))}
                  className={inputCls}
              ...
              flags.map((f: any) => {
                ...
                return (
                  <tr key={f.id} className="hover:bg-[hsl(var(--secondary)/0.3)] transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs text-[hsl(var(--muted-foreground))] max-w-[200px] truncate">
                      {f.description || '—'}
                    </td>
                    <td className="px-4 py-3">
                      <button
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => {
                            if (confirm('آیا از حذف این feature flag مطمئن هستید؟'))
