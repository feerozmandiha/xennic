  ...
  return (
    <div>
      <FLabel required={required}>
        {label}
        {unit && <span className="text-[hsl(var(--muted-foreground))] font-normal"> ({unit})</span>}
      </FLabel>
      <input
        type="number"
        step="any"
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-1 p-1 rounded-[var(--radius-lg)] bg-[hsl(var(--muted)/0.4)]">
            <button type="button" className={tabCls('basic')} onClick={() => setSection('basic')}>
              اطلاعات پایه
            </button>
            <button type="button" className={tabCls('tou')} onClick={() => setSection('tou')}>
              اوج/میان/کم‌بار
            </button>
            <button
              type="button"
              className={tabCls('network')}
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <FInput
                  ...
                  value={form.amount_rials}
                  onChange={set('amount_rials')}
                  unit="ریال"
                  hint="اختیاری"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                />
                <FSelect
                  label="منطقه آب‌وهوایی"
                  value={form.climate_zone}
                  onChange={set('climate_zone')}
                  options={[
              ...
              {(isIndustr || form.subscriber_type === 'commercial') && (
                <div className="p-3 rounded-[var(--radius-lg)] bg-[hsl(var(--secondary)/0.5)] space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <FInput
                      label="قرائت ماکسیمتر"
                      value={form.maximeter_kw}
                      onChange={set('maximeter_kw')}
                      unit="kW"
                      hint="از قبض"
                    />
                    <FInput
                      label="توان اوج دوره"
                      value={form.current_peak_kw}
                      onChange={set('current_peak_kw')}
                      unit="kW"
                      hint="peak demand"
                    />
                    <FInput
                      label="توان قراردادی"
                      value={form.contract_kw}
                      onChange={set('contract_kw')}
                      unit="kW"
                      hint="مندرج در قرارداد"
                    />
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <FInput
                  label="مصرف راکتیو"
                  value={form.kvarh_consumed}
                  onChange={set('kvarh_consumed')}
                  unit="kVArh"
                  hint="اختیاری"
                />
              </div>
          ...
          {section === 'tou' && (
            <div className="space-y-3">
              <div className="p-3 rounded-[var(--radius-lg)] bg-[hsl(var(--warning)/0.05)] border border-[hsl(var(--warning)/0.15)] text-xs text-[hsl(var(--muted-foreground))] leading-relaxed">
                <p className="font-semibold text-[hsl(var(--warning))] mb-1">
                  📋 راهنمای ساعات TOU (توانیر ۱۴۰۳)
                </p>
                <p>
                  • <strong>اوج‌بار:</strong> تابستان ۱۷–۲۱ | زمستان ۷–۱۰ و ۱۷–۲۱
                </p>
                <p>
                  • <strong>کم‌بار:</strong> ۲۳–۷ (همه فصول)
                </p>
                <p>
                  • <strong>میان‌بار:</strong> سایر ساعات
                </p>
                <p>
                  • <strong>اوج جمعه:</strong> ساعات اوج روزهای جمعه (نرخ کمتر)
                </p>
                <p className="mt-1 text-[hsl(var(--primary))]">
                  اگر ندارید خالی بگذارید — سیستم از نرخ تک‌نرخی استفاده می‌کند
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <FInput
                  label="مصرف میان‌بار"
                  value={form.mid_kwh}
                  onChange={set('mid_kwh')}
                  unit="kWh"
                  hint="ساعات میانی"
                />
                <FInput
                  label="مصرف کم‌بار"
                  value={form.off_peak_kwh}
                  onChange={set('off_peak_kwh')}
                  unit="kWh"
                  hint="شب ۲۳–۷"
                />
                <FInput
                  label="اوج‌بار جمعه"
                  value={form.peak_kwh_friday}
                  onChange={set('peak_kwh_friday')}
                  unit="kWh"
                  hint="اختیاری"
                />
              </div>
              ...
              {(form.peak_kwh || form.mid_kwh || form.off_peak_kwh) && (
                <div className="flex items-center justify-between p-2 rounded bg-[hsl(var(--primary)/0.06)] border border-[hsl(var(--primary)/0.2)] text-xs">
                  ...
                  <span className="text-[hsl(var(--muted-foreground))]">
                    از {parseFloat(form.kwh_consumed || '0').toLocaleString('fa-IR')} kWh
                  </span>
                </div>
              )}
            </div>
          ...
          {section === 'network' && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                <FInput
                  label="ترانسفورماتور"
                  value={form.transformer_kva}
                  onChange={set('transformer_kva')}
                  unit="kVA"
                  hint="مثال: 630"
                />
                <FInput
                  ...
                  onChange={set('cable_length_m')}
                  unit="متر"
                  hint="مثال: 80"
                />
                <FInput
                  label="سطح مقطع کابل"
                  value={form.cable_size_mm2}
                  onChange={set('cable_size_mm2')}
                  unit="mm²"
                  hint="مثال: 240"
                />
              </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 p-3 rounded-[var(--radius-lg)] bg-[hsl(var(--success)/0.08)] border border-[hsl(var(--success)/0.25)]">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-[hsl(var(--muted-foreground))]">
            روش:{' '}
            <span className="font-mono">{METHOD_LABEL[ocr?.method] ?? ocr?.method ?? '—'}</span>
            {result?.file?.name && ` | فایل: ${result.file.name}`}
            {sum.climate_zone &&
              sum.climate_zone !== 'moderate' &&
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <SectionCard title="داده‌های قبض" icon={FileText} color="text-[hsl(var(--primary))]">
          ...
          <KVRow
            label="منطقه"
            value={
              norm.climate_zone === 'hot'
                ? 'گرمسیری'
                : norm.climate_zone === 'cold'
                  ? 'سردسیری'
                  : norm.climate_zone
                    ? 'معتدل'
                    : null
          ...
          <KVRow
            label="نوع قرارداد"
            value={norm.contract_type === 'tou' ? 'TOU (اوج/میان/کم‌بار)' : norm.contract_type}
          />
        ...
        {cons.kwh_total && (
          <SectionCard title="شاخص‌های مصرف" icon={BarChart3} color="text-[hsl(var(--primary))]">
            ...
            <KVRow
              ...
              highlight={
                cons.load_factor >= 0.65 ? 'good' : cons.load_factor >= 0.45 ? 'warn' : 'bad'
              }
            />
            {/* TOU breakdown */}
            ...
            {cons.kwh_off_peak && (
              <KVRow label="مصرف کم‌بار" value={cons.kwh_off_peak} unit="kWh" />
            )}
            {eff?.overall_score != null && (
              <>
                <KVRow
                  label="امتیاز کارایی"
                  value={`${eff.overall_score}/100`}
        ...
        {(pf.measured != null || pf.status) && (
          <SectionCard
            title="ضریب قدرت و جبران‌سازی"
            icon={TrendingUp}
            color={
              pf.below_threshold ? 'text-[hsl(var(--destructive))]' : 'text-[hsl(var(--success))]'
            }
          >
            <KVRow
              label="ضریب قدرت اندازه‌گیری"
              value={pf.measured}
            ...
            <KVRow
              label="جریمه ضریب قدرت"
              value={pf.penalty_rials}
              unit="ریال"
        ...
        {cost.total_rials && (
          ...
          >
            ...
            <KVRow
              label="جریمه تجاوز توان"
              value={cost.overload_cost_rials}
              unit="ریال"
            ...
            {cost.tou_breakdown && (
              <div className="mt-2 p-2 rounded bg-[hsl(var(--secondary)/0.5)] text-xs space-y-1">
                <p className="font-semibold text-[hsl(var(--muted-foreground))] text-[10px] uppercase">
                  تفکیک TOU
                </p>
            ...
            {lf.converged === false ? (
              <>
                ...
                {lf.simple_estimate?.voltage_drop_pct != null && (
                  <>
                    <KVRow
                      label="افت ولتاژ (تخمین)"
                      value={lf.simple_estimate.voltage_drop_pct}
                      unit="%"
                      highlight={lf.simple_estimate.voltage_ok ? 'good' : 'bad'}
                    />
              ...
              <>
                <KVRow
                  label="ولتاژ"
                  value={lf.voltage_pu ? `${lf.voltage_pu} pu` : null}
                  highlight={lf.voltage_ok ? 'good' : 'bad'}
                />
                <KVRow
                  label="وضعیت ولتاژ"
                  value={lf.voltage_ok ? 'در محدوده مجاز' : 'خارج از محدوده'}
                  highlight={lf.voltage_ok ? 'good' : 'bad'}
                />
                <KVRow
                  label="تلفات توان"
                  value={lf.loss_kw}
                  unit="kW"
                  highlight={(lf.loss_pct ?? 0) > 3 ? 'warn' : 'good'}
                />
                <KVRow label="تلفات %" value={lf.loss_pct?.toFixed(2)} unit="%" />
                <KVRow label="تلفات ماهانه" value={lf.loss_kwh_month} unit="kWh" />
                <KVRow
                  label="بارگذاری ترانس"
                  value={lf.transformer_loading_pct}
                  unit="%"
                  highlight={(lf.transformer_loading_pct ?? 0) > 80 ? 'bad' : 'good'}
                />
              </>
            )}
          </SectionCard>
      ...
      {warns.length > 0 && (
        <div className="space-y-2">
          {warns.map((w: string, i: number) => (
            <div
              key={i}
              className="flex items-start gap-2 p-3 rounded-[var(--radius-lg)] bg-[hsl(var(--warning)/0.06)] border border-[hsl(var(--warning)/0.2)] text-xs"
            >
...
export function BillAnalyzer() {
  ...
  function buildPrefill(): Partial<FormState> {
    ...
    return {
      ...
      subscriber_type:
        norm.subscriber_type === 'industrial'
          ? 'industrial_mv'
          : (norm.subscriber_type ?? 'residential'),
          ...
          {ocrRaw?.ocr && (
            <div className="p-3 rounded-[var(--radius-lg)] bg-[hsl(var(--warning)/0.06)] border border-[hsl(var(--warning)/0.2)] text-xs space-y-1">
              ...
              {ocrRaw.ocr.normalized?.amount_rials && (
                <p>
                  مبلغ خوانده‌شده:{' '}
                  <strong>{ocrRaw.ocr.normalized.amount_rials.toLocaleString('fa-IR')} ریال</strong>
                </p>
              )}
            </div>
          )}
          ...
          {[
            {
              icon: FileText,
              title: 'OCR + Groq AI',
              desc: 'PDF text layer → regex → Groq AI fallback برای استخراج دقیق داده قبض',
            },
            {
              icon: Zap,
              title: 'تعرفه کامل ۱۴۰۳',
              desc: 'TOU اوج/میان/کم‌بار، منطقه گرمسیری/سردسیری، ماکسیمتر، ratchet',
            },
            {
              icon: Cpu,
              title: 'pandapower',
              desc: 'Load Flow Newton-Raphson — ولتاژ، تلفات، بارگذاری ترانس، پیشنهادات',
            },
