...
async function main() {
  ...
  const WorkspaceGuard = require(
    join(distDir, 'modules/rbac/infrastructure/guards/workspace.guard.js'),
  ).WorkspaceGuard;
  const PermissionsGuard = require(
    join(distDir, 'modules/rbac/infrastructure/guards/permissions.guard.js'),
  ).PermissionsGuard;
  const JwtAuthGuard = require(
    join(distDir, 'modules/auth/infrastructure/guards/jwt-auth.guard.js'),
  ).JwtAuthGuard;
