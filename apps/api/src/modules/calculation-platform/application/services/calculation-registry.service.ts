import { Injectable, Logger, Inject, ConflictException, NotFoundException, BadRequestException } from '@nestjs/common';
...
export class CalculationRegistryService {
  ...
  async createCategory(data: { name: string; slug: string; description?: string | null; parentId?: string | null; icon?: string | null; sortOrder?: number }): Promise<CalculationCategoryEntity> {
  ...
  async updateCategory(id: string, data: Partial<{ name: string; description: string | null; icon: string | null; sortOrder: number }>): Promise<CalculationCategoryEntity> {
  ...
  async createDefinition(data: {
    categoryId: string; slug: string; name: string; description?: string | null;
    standard?: string | null; standardRef?: string | null; enabled?: boolean;
    aiReview?: boolean; certificate?: boolean; metadata?: Record<string, unknown>;
  ...
  async getAllDefinitions(options?: { categoryId?: string; enabled?: boolean }): Promise<CalculationDefinitionEntity[]> {
    return this.repo.findAllDefinitions(options);
  }

  async updateDefinition(id: string, data: Partial<{
    name: string; description: string | null; standard: string | null; standardRef: string | null;
    enabled: boolean; aiReview: boolean; certificate: boolean; metadata: Record<string, unknown>;
  }>): Promise<CalculationDefinitionEntity> {
  ...
  async createVersion(data: {
    definitionId: string; version: string; dslDefinition: DslDefinition; changeLog?: string | null; createdBy: string;
  ...
  async createFormula(data: {
    definitionId?: string | null; versionId?: string | null; name: string;
    expression: string; description?: string | null; returnType?: string; metadata?: Record<string, unknown>;
  ...
  async updateFormula(id: string, data: Partial<{ name: string; expression: string; description: string | null; returnType: string; metadata: Record<string, unknown> }>): Promise<FormulaDefinitionEntity> {
  ...
  async createVariable(data: {
    formulaId: string; name: string; label?: string | null; type?: 'number' | 'string' | 'boolean' | 'enum' | 'table';
    unitId?: string | null; required?: boolean; defaultValue?: unknown; minValue?: number; maxValue?: number;
    enumValues?: string[] | null; description?: string | null; sortOrder?: number;
