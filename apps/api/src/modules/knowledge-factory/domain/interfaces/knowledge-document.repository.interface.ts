...
export interface IKnowledgeDocumentRepository {
  ...
  findByWorkspace(workspaceId: string, offset?: number, limit?: number): Promise<{ data: KnowledgeDocument[]; total: number }>;
  ...
  classifyDocument(documentId: string, classification: Record<string, unknown>): Promise<KnowledgeDocument>;
