...
@Injectable()
export class RabbitMQMessageQueue implements IMessageQueue, IDeadLetterQueue {
  private readonly logger = new Logger(RabbitMQMessageQueue.name);
  private readonly handlers = new Map<string, IMessageHandler>();
  private readonly deadLetterStore: DeadLetterRecord[] = [];

  constructor(private readonly rabbitMQ: RabbitMQService) {}

  async publish<T>(envelope: Omit<MessageEnvelope<T>, 'timestamp' | 'retryCount'>): Promise<void> {
    ...
    await this.rabbitMQ.sendToQueue(message.messageType, message);
  }
