...
export class ProviderStatisticsEntity {
  ...
  static create(
    ...
    data: {
      ...
      avgLatencyMs?: number; p50LatencyMs?: number;
      p95LatencyMs?: number; p99LatencyMs?: number;
      successRate?: number;
    },
    ...
    return new ProviderStatisticsEntity(
      ...
      data.successCount, data.failureCount, data.totalRequests,
      data.avgLatencyMs ?? null, data.p50LatencyMs ?? null,
      data.p95LatencyMs ?? null, data.p99LatencyMs ?? null,
  ...
  static reconstitute(data: {
    ...
    avg_latency_ms: number | null; p50_latency_ms: number | null;
    p95_latency_ms: number | null; p99_latency_ms: number | null;
    success_rate: number | null; time_period: string;
    recorded_at: Date; created_at: Date;
  }): ProviderStatisticsEntity {
    return new ProviderStatisticsEntity(
      data.id, data.provider_id,
      data.success_count, data.failure_count, data.total_requests,
      data.avg_latency_ms, data.p50_latency_ms,
      data.p95_latency_ms, data.p99_latency_ms,
      data.success_rate, data.time_period as TimePeriod,
