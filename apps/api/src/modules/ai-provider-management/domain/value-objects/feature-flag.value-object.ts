export type FeatureFlagType =
  | 'vision' | 'embedding' | 'reasoning' | 'audio'
  | 'ocr' | 'image_generation' | 'translation'
  | 'tools' | 'json_mode';
...
export class FeatureFlag {
  ...
  static reconstitute(data: {
    id: string; workspace_id: string | null; provider_id: string | null;
    model_id: string | null; feature: string;
    enabled: boolean; created_by: string; created_at: Date;
  }): FeatureFlag {
    return new FeatureFlag(
      data.id, data.workspace_id, data.provider_id, data.model_id,
      data.feature as FeatureFlagType, data.enabled,
