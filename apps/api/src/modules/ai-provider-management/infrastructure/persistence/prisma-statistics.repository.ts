...
import { ProviderStatisticsEntity, TimePeriod } from '../../domain/entities/provider-statistics.entity.js';

const prisma = new PrismaClient();

@Injectable()
export class PrismaStatisticsRepository implements IStatisticsRepository {
  async findByProviderId(providerId: string, timePeriod: TimePeriod): Promise<ProviderStatisticsEntity | null> {
  ...
  async save(stats: ProviderStatisticsEntity): Promise<void> {
    await prisma.ai_provider_statistics.create({ data: {
      id: stats.id,
  ...
  async getAggregatedStats(providerId: string): Promise<{
    successRate: number; avgLatency: number; totalRequests: number;
  } | null> {
