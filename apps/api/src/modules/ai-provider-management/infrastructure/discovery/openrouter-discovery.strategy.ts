...
export class OpenRouterDiscoveryStrategy implements IDiscoveryStrategy {
  ...
  private async fetchModels(apiKey: string, baseUrl: string): Promise<DiscoveryResult['models']> {
    try {
      ...
      return models.map((m: any) => ({
        modelId: m.id, displayName: m.name || m.id,
        modelType: m.architecture?.modality === 'embedding' ? 'embedding' : m.architecture?.modality === 'image' ? 'image' : 'chat',
        contextWindow: m.context_length,
        supportsStreaming: true, supportsTemperature: true, supportsTopP: true,
        supportsVision: m.architecture?.modality === 'text+image',
        supportsTools: !m.architecture?.modality?.includes('image') && !m.architecture?.modality?.includes('embedding'),
    ...
    } catch { return this.getTopModels(); }
  }
  ...
  private getTopModels(): DiscoveryResult['models'] {
    const models = [
      ...
      'deepseek/deepseek-r1', 'deepseek/deepseek-chat', 'cohere/command-r-plus',
    ];
    return models.map(id => ({ modelId: id, displayName: id, modelType: 'chat' as const, supportsStreaming: true, supportsTemperature: true, supportsTopP: true }));
  }
}
