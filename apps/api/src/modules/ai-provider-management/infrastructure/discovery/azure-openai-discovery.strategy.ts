...
export class AzureOpenAIDiscoveryStrategy implements IDiscoveryStrategy {
  ...
  async discover(apiKey: string, baseUrl: string): Promise<DiscoveryResult> {
    if (!baseUrl) throw new Error('Azure OpenAI requires a base URL (e.g., https://your-resource.openai.azure.com)');
...
const DEFAULT_MODELS: DiscoveryResult['models'] = [
  { modelId: 'gpt-4o', displayName: 'GPT-4o', modelType: 'chat', contextWindow: 128000, maxOutputTokens: 16384, supportsTools: true, supportsJson: true, supportsStreaming: true, supportsVision: true, supportsFunctionCalling: true, supportsTemperature: true, supportsTopP: true },
  { modelId: 'gpt-4o-mini', displayName: 'GPT-4o Mini', modelType: 'chat', contextWindow: 128000, maxOutputTokens: 16384, supportsTools: true, supportsJson: true, supportsStreaming: true, supportsVision: true, supportsFunctionCalling: true, supportsTemperature: true, supportsTopP: true },
  { modelId: 'gpt-4-turbo', displayName: 'GPT-4 Turbo', modelType: 'chat', contextWindow: 128000, maxOutputTokens: 4096, supportsTools: true, supportsJson: true, supportsStreaming: true, supportsTemperature: true, supportsTopP: true },
  { modelId: 'text-embedding-3-small', displayName: 'Text Embedding 3 Small', modelType: 'embedding', contextWindow: 8191, supportsEmbedding: true },
  { modelId: 'text-embedding-3-large', displayName: 'Text Embedding 3 Large', modelType: 'embedding', contextWindow: 8191, supportsEmbedding: true },
];
