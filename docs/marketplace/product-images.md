...
> **بعد از اعمال مهاجرت حتماً کلاینت Prisma را دوباره تولید کنید:**
> `pnpm prisma generate --schema prisma/schema.prisma`
> تایپ‌های `product_images` و `products.images` فقط از همین مسیر ساخته می‌شوند و
> چون `pnpm install` محتوای پکیج‌ها را از store بازیابی می‌کند، کلاینت تولیدشده پس
> از هر نصب دوباره پاک می‌شود و بیلد `apps/api` با
> `Property 'product_images' does not exist` می‌شکند.
