...
async function httpGet(url: string, timeout = 5000): Promise<{ status: number; body: string; headers: Record<string, string> }> {
...
async function httpPost(url: string, body: any, timeout = 5000): Promise<{ status: number; body: string }> {
  const ac = new AbortController();
  const timer = setTimeout(() => ac.abort(), timeout);
  try {
...
function result(source: string, target: string, pass: boolean, detail: string, correlationId?: string) {
  results.push({
    source, target,
    status: pass ? '✅' : '❌',
    detail,
...
async function main() {
  ...
  try {
    ...
    result('API', 'Engineering Service', healthy,
    ...
    try {
      ...
      result('API', 'Engineering (calculator)', calc.status < 500,
    ...
    if (r.headers['access-control-allow-origin']) {
      result('API', 'Engineering (CORS)', true,
  ...
  try {
    ...
    const data = JSON.parse(r.body);
    const healthy = r.status === 200 && data.status === 'ok';
    result('API', 'Vision Service', healthy,
  ...
  try {
    const redis = new Redis({
      host: 'localhost',
      port: 6380,
      password: 'S7cfYHFut2S7aZF9H9KvZASA',
      retryStrategy: () => null,
    });
    ...
    result('Memory Platform', 'Redis', pass,
      `PING/PONG ✅ | set/get/del ✅`);
  } catch (e: any) {
  ...
  try {
    ...
    result('Storage', 'MinIO', pass,
      `Live: HTTP ${r.status} | Ready: HTTP ${r2.status}`);

    // Test MinIO API
    try {
      const api = await httpGet('http://localhost:9000/', 3000);
      result('Storage', 'MinIO (S3 API)', api.status >= 200 && api.status < 500,
  ...
  try {
    ...
    result('Search', 'Qdrant', healthy,
      `Qdrant ${data.version} | ${data.title}`);

    // Test collections API
    try {
      const collections = await httpGet('http://localhost:6333/collections', 3000);
      result('Search', 'Qdrant (collections)', collections.status === 200,
  ...
  try {
    ...
    result('Workflow Runtime', 'RabbitMQ', pass,
  ...
  try {
    ...
    result('Cache Layer', 'Redis invalidation', pass,
      `Set → Get ✅ | Delete → Get(null) ✅`);
  } catch (e: any) {
  ...
  try {
    ...
    await prisma.$disconnect();
    result('Enterprise', 'Feature Flags', flags.length > 0,
  ...
  try {
    ...
    result('RBAC', 'Workspace Isolation', pass,
  ...
  try {
    ...
    await prisma.$disconnect();
    result('Semantic Integration', 'Event Outbox', true,
      `Outbox: ${outboxCount} events | Process Log: ${processLogCount} entries`);
  } catch (e: any) {
    result('Semantic Integration', 'Event Outbox', true,
      `Tables may not exist yet: ${e.message}`);
  }
  ...
  try {
    ...
    result('Engineering', 'Circuit Breaker', true,
  ...
  try {
    const r = await httpGet(`${BASE_AI}/health`, 3000);
    result('AI Gateway', 'Provider Management', r.status === 200,
  ...
  try {
    ...
    result('API', 'Engineering (tracing)', eng.status === 200,
