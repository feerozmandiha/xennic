...
export class Flow {
  ...
  private async handleMessage(
    ...
    switch (s.state.name) {
      ...
      case 'consult_name': {
        ...
        if (st.channel === 'pv') {
          ...
          await api.sendMessage(
            chatId,
            'شماره تماس را بفرستید (مثال: ۰۹۱۲۳۴۵۶۷۸۹):',
      ...
      case 'consult_phone': {
        ...
        if (!phone) {
          await api.sendMessage(chatId, '⚠️ شماره معتبر نیست. لطفاً شماره موبایل را به شکل ۰۹۱۲۳۴۵۶۷۸۹ بفرستید.');
          return;
        }
        const st = s.state;
        s.state = {
  ...
  private async processBill(
    ...
    // محدودسازی نرخ فایل
    const now = Date.now();
    const hits = (this.rateFiles.get(String(chatId)) ?? []).filter(
      (t) => now - t < 3600_000,
    );
    if (hits.length >= 5) {
      await api.sendMessage(chatId, '⏳ تعداد فایل‌های ارسالی در یک ساعت بیش از حد مجاز است. لطفاً بعداً تلاش کنید.');
      return;
    }
    hits.push(now);
    ...
    const waitMsg = await api.sendMessage(chatId, '⏳ در حال خواندن قبض… (معمولاً کمتر از ۳۰ ثانیه)');
    s.state = { name: 'idle' };
  ...
  private async finalizeManual(
    ...
    if (bill.consumptionKwh === undefined) {
      await api.sendMessage(chatId, '⚠️ «مصرف» (کیلووات‌ساعت) پیدا نشد. دوباره با فرمول «مصرف: ۲۶۵» بفرستید.');
      return;
    }
    s.bill = bill;
    s.state = { name: 'idle' };
  ...
  private async runAnalysis(
    ...
    try {
      ...
      await api.sendMessage(chatId, `⚠️ تحلیل ناموفق بود: ${err instanceof Error ? err.message : 'خطای نامشخص'}`);
      return;
    }
  ...
  private async applyFix(
    ...
    const entry = Object.entries(FIELD_LABELS).find(([, fa]) => fa.includes(label) || label.includes(fa.split(' ')[0]));
    if (!entry || value === null) {
      await api.sendMessage(
        chatId,
        `فیلدهای قابل اصلاح:\n${Object.values(FIELD_LABELS).map((v) => '• ' + v).join('\n')}\nفرمت: «مصرف: ۲۶۵»`,
      );
      return;
    }
  ...
  private async handleCallback(
    ...
    switch (data) {
      ...
      case CB.back:
        s.state = { name: 'idle' };
        await api.sendMessage(chatId, 'باشه. اگر خواستید، دکمه «🎧 درخواست مشاوره» هنوز زیر گزارش هست.');
        return;
      case CB.fix: {
  ...
  private async finalizeConsultation(
    ...
    if (open.length >= 3) {
      await api.sendMessage(chatId, '⚠️ سه درخواست باز دارید؛ پس از پیگیری آن‌ها درخواست جدید ثبت کنید.');
      return;
    }
...
export function formatBillTable(bill: BillData, confPct: number): string {
  const lines: string[] = [
    ...
    bill.paymentStatus ? `وضعیت: ${bill.paymentStatus === 'paid' ? 'پرداخت شده ✅' : 'پرداخت نشده ⏳'}` : '',
  ];
...
export function formatAnalysis(r: ReportModel): string {
  ...
  const extra: string[] = [];
  if (r.peakSurchargeRials) extra.push(`برآورد جریمه اوج‌بار: ${faNumber(r.peakSurchargeRials)} ریال`);
