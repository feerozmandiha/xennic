...
export function buildHtmlReport(
  ...
  const billRows = [
    ...
    row('دوره قبض', `${faDate(b.periodFrom)} تا ${faDate(b.periodTo)}${b.periodDays ? ` (${faNumber(b.periodDays)} روز)` : ''}`),
    row('شاخص قبلی / فعلی', b.prevReading !== undefined && b.curReading !== undefined ? `${faNumber(b.prevReading)} / ${faNumber(b.curReading)}` : '—'),
    row('مصرف دوره', b.consumptionKwh !== undefined ? `${faNumber(b.consumptionKwh)} کیلووات‌ساعت` : '—', (conf['consumptionKwh'] ?? 1) < 0.7),
    row('میانگین ماهانه', `${faNumber(report.monthlyAvgKwh)} کیلووات‌ساعت`),
    row('قدرت / آمپراژ', b.demandKw !== undefined || b.ampere !== undefined ? `${b.demandKw ? faNumber(b.demandKw) + ' کیلووات' : ''} ${b.ampere ? faNumber(b.ampere) + ' آمپر' : ''}`.trim() : '—'),
    ...(b.tou
      ? [row('کم‌باری / میان‌باری / اوج‌باری', `${faNumber(b.tou.low ?? 0)} / ${faNumber(b.tou.mid ?? 0)} / ${faNumber(b.tou.peak ?? 0)} کیلووات‌ساعت`)]
      : []),
  ].join('');

  const moneyRows = [
    row('بهای انرژی (روی قبض)', b.energyChargeRials !== undefined ? `${faNumber(b.energyChargeRials)} ریال` : '—'),
    ...
    row('بیمه / آئونمان', b.insuranceRials !== undefined || b.subscriptionRials !== undefined ? `${faNumber((b.insuranceRials ?? 0) + (b.subscriptionRials ?? 0))} ریال` : '—'),
    row('جمع کل قابل پرداخت', b.totalRials !== undefined ? `${faNumber(b.totalRials)} ریال` : '—'),
    row('وضعیت پرداخت', b.paymentStatus === 'paid' ? 'پرداخت شده ✅' : b.paymentStatus === 'unpaid' ? 'پرداخت نشده ⏳' : '—'),
  ].join('');
