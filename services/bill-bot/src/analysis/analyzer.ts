...
export function analyzeBill(bill: BillData, opts: AnalyzeOptions = {}): ReportModel {
  ...
  if (bill.tou?.peak) {
    peakSurchargeRials = peakSurcharge(
      bill.tou.peak,
      overPattern,
      zone,
      season,
  ...
  for (const key of ['consumptionKwh', 'periodDays', 'region', 'energyChargeRials']) {
    ...
    if (c !== undefined && c < 0.7) {
      warnings.push(`اطمینان استخراج فیلد «${key}» پایین است (${Math.round(c * 100)}٪) — در صورت نادرستی، با دکمه اصلاح آن را ویرایش کنید.`);
    }
  }
...
function buildRecommendations(ctx: RecCtx): Recommendation[] {
  ...
  if (monthlyAvgKwh > patternLimitKwh) {
    const excessKwh = (monthlyAvgKwh - patternLimitKwh) * ctx.bill.consumptionKwh! /
      Math.max(monthlyAvgKwh, 1);
    const save = Math.round(excessKwh * Math.max(ctx.effectiveRateRials - supplyCostRials * 0.5, 0));
    recs.push({
      id: 'R1',
  ...
  return recs
    .sort((a, b) => (b.saveRialsPerPeriod ?? 0) - (a.saveRialsPerPeriod ?? 0))
    .slice(0, 5);
}
