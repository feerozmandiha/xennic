...
export function extractBillFields(rawText: string): BillData {
  ...
  set(
    'mobile',
    firstMatch(t, /(?:تلفن\s*همراه|همراه|موبایل)[:\s.]*\s*(0?9\d{9})/) ??
      firstMatch(t, /(0?9\d{9})/),
    0.65,
  );
